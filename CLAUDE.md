...
| `SaleController` | Compras por funcionário e por período |
...
Rotas em `routes/api.php` — todas protegidas por `auth:sanctum` exceto `/login`.
...
├── router.tsx                       # createBrowserRouter com ProtectedRoute
├── lib/
...
- **Partner** — funcionário: `cpf` (11 dígitos, único), `matricula` (único), `limcred`, `bloqueado`, `alterado`
- **Sale** — compra vinculada ao Partner via `cpf` (string, não FK inteira); campos: `codfilial`, `numnota`, `dtsaida`, `vltotal`, `qrcodenfce`
- **Filial** — filiais/lojas
- **PartnerError** — registros de CSV que falharam, para correção manual
- **ChangeLog** — auditoria; toda escrita bem-sucedida insere registro com `usuario`, `operacao`, `descricao`
- **User** — autenticação Laravel/Sanctum
