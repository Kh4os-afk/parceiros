...
interface Partner {
    id: number; nome: string; cpf: string; matricula: string;
...
export default function ListPage() {
    ...
    async function exportarExcel() {
        ...
        try {
            ...
            ws["!cols"] = [{ wch:12 },{ wch:35 },{ wch:16 },{ wch:18 },{ wch:12 }];
    ...
    const kpis = [
        ...
        { label:"Ativos",            value:summary.ativos,     icon:ShieldCheck, color:T.green,  money:false },
        { label:"Bloqueados",        value:summary.bloqueados, icon:ShieldOff,   color:summary.bloqueados>0?T.red:"#94a3b8", money:false },
        { label:"Lim. Médio Mensal", value:summary.lim_medio,  icon:CreditCard,  color:T.purple, money:true  },
        { label:"Total em Crédito",  value:summary.lim_total,  icon:CreditCard,  color:T.cyan,   money:true  },
    ];
    ...
    return (
        ...
        >
            {/* ── Header ── */}
            <motion.div variants={rise} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                    <p className="text-[0.55rem] font-black uppercase tracking-[0.28em] text-muted-foreground mb-0.5">Gestão</p>
                    <h1 className="text-xl font-black uppercase tracking-tight text-foreground">Funcionários</h1>
                </div>
                <div className="flex gap-2">
                    <Button variant="outline" size="sm" className="rounded-none gap-1.5 text-xs"
            ...
            {/* ── KPI Strip ── */}
            <motion.div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3" variants={stagger(0.08)}>
                {kpis.map(({ label, value, icon: Icon, color, money }, i) => (
                    <motion.div
                        ...
                        style={{ border:`1px solid ${T.border}`, boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}
                        whileHover={{ y:-3, boxShadow:`0 8px 24px rgba(0,0,0,0.08), 0 0 0 1px ${color}25`, transition:{ type:"spring", stiffness:300, damping:22 } }}
                    >
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <p className="text-[0.52rem] font-black uppercase tracking-[0.16em] text-muted-foreground leading-tight">{label}</p>
                        </div>
                        <p className="text-xl font-black tabular-nums text-foreground leading-none mb-2">
                            ...
                            <CountUp value={value} decimals={money?2:0} duration={0.8} delay={80+i*40}/>
                        </p>
                        <div className="mt-auto h-[2px] w-full overflow-hidden" style={{ background:`${color}10` }}>
                ...
                <div className="flex flex-wrap items-center gap-2 px-4 py-3.5" style={{ borderBottom:`1px solid ${T.border}` }}>
                    <div className="relative flex-1 min-w-[200px] max-w-sm">
                        ...
                        <input value={search} onChange={e => setSearch(e.target.value)}
                               ...
                               style={{ border:`1px solid ${T.border}`, color:"var(--foreground)" }}
                    ...
                    <button onClick={exportarExcel} disabled={exporting}
                        ...
                        <Download size={10}/>{exporting?"Exportando…":"Exportar Excel"}
                    </button>
                </div>
                            ...
                            <tr style={{ background:"#f9fafb", borderBottom:`1px solid ${T.border}` }}>
                                {([["matricula","Matrícula"],["nome","Nome"],["cpf","CPF"],["limcred","Lim. Mensal"],["bloqueado","Status"]] as [SortField,string][])
                                    ...
                                    <th key={field} onClick={() => toggleSort(field)}
                                        ...
                                        <span className={`flex items-center gap-1 text-[0.62rem] font-black uppercase tracking-widest`}
                                              style={{ color: sortBy===field ? T.cyan : "var(--muted-foreground)" }}>
                                            {label}
                                            {sortBy===field
                                ...
                                <th className="px-4 py-3 text-right text-[0.62rem] font-black uppercase tracking-widest text-muted-foreground">Ações</th>
                            </tr>
                        </thead>
                        <tbody>
                            {initialLoading ? (
                                ...
                                <tr key={p.id} className="group transition-colors hover:bg-[#fafafa]"
                                    ...
                                    <td className="px-4 py-2 relative">
                                        <div className={`absolute left-0 top-1.5 bottom-1.5 w-[3px] transition-opacity ${p.bloqueado?"opacity-70":"opacity-0 group-hover:opacity-30"}`}
                                             style={{ background: p.bloqueado ? T.red : T.cyan }}/>
                                        <span className="text-sm font-mono text-muted-foreground tabular-nums">{p.matricula||"—"}</span>
                                    </td>
                                    ...
                                    <td className="px-4 py-2">
                                        <span className="text-sm font-mono text-muted-foreground tracking-wider">{formatCPF(p.cpf)}</span>
                                    </td>
                                    <td className="px-4 py-2">
                                        <span className="text-sm font-black tabular-nums" style={{ color:T.cyan }}>
                                            <span className="text-[0.6em] opacity-40 mr-0.5">R$</span>
                                            <CountUp value={p.limcred} decimals={2} duration={0.5} delay={80}/>
                                        </span>
                                    </td>
                                    <td className="px-4 py-2">
                                        {p.bloqueado ? (
                                            <span className="inline-flex items-center gap-1.5 text-[0.6rem] font-black uppercase tracking-wider" style={{ color:T.red }}>
                                            ...
                                            <span className="inline-flex items-center gap-1.5 text-[0.6rem] font-black uppercase tracking-wider" style={{ color:T.green }}>
                                                <span className="relative flex h-1.5 w-1.5">
                                    ...
                                    {isAdmin && (
                                        <td className="px-4 py-2">
                                            <span className="text-sm text-muted-foreground">{p.empresa?.nome??"—"}</span>
                                        </td>
                                    )}
                                    <td className="px-4 py-2 text-right">
                                        <div className="inline-flex gap-1 opacity-40 group-hover:opacity-100 transition-opacity">
                                            <button onClick={() => navigate(`/funcionarios/${p.id}`)} title="Ver detalhes"
                                                    className="w-7 h-7 flex items-center justify-center transition-colors"
                                                    style={{ border:`1px solid ${T.border}`, color:"var(--muted-foreground)" }}
                                                    onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor=T.cyan; (e.currentTarget as HTMLElement).style.color=T.cyan; }}
                                                    onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor=T.border; (e.currentTarget as HTMLElement).style.color="var(--muted-foreground)"; }}>
                                                <ReceiptText size={11}/>
                                            </button>
                                            <button onClick={() => navigate(`/funcionarios/${p.id}/editar`)} title="Editar"
                                                    className="w-7 h-7 flex items-center justify-center transition-colors"
                                                    style={{ border:`1px solid ${T.border}`, color:"var(--muted-foreground)" }}
                                                    onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor=T.cyan; (e.currentTarget as HTMLElement).style.color=T.cyan; }}
                                                    onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor=T.border; (e.currentTarget as HTMLElement).style.color="var(--muted-foreground)"; }}>
