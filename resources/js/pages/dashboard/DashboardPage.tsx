                ...
                <div className="relative grid grid-cols-2 md:grid-cols-4 divide-(--border) [&>*]:border-r [&>*:nth-child(2n)]:border-r-0 md:[&>*]:border-r md:[&>*:nth-child(2n)]:border-r md:[&>*:nth-child(4n)]:border-r-0 [&>*:nth-child(n+3)]:border-t md:[&>*:nth-child(n+3)]:border-t-0">
                    {kpis.map(
                        ({
                            ...
                            >
                                <div
                                    className={`absolute top-0 left-0 right-0 h-0.5 transition-opacity duration-200 ${highlight ? "opacity-100 bg-(--primary)" : "opacity-0 group-hover:opacity-50 bg-(--primary)"}`}
                                />
                                ...
                                <p
                                    className={`font-black tabular-nums leading-none transition-colors ${highlight ? "text-2xl text-(--primary)" : `text-xl ${color === "amber" && value > 0 ? "text-amber-500" : "text-(--foreground) group-hover:text-(--primary)"}`}`}
                                >
                                    {decimals > 0 && (
                                        <span className="text-[0.45em] font-bold opacity-50 mr-0.5">
                                            R$
                                        </span>
                                    )}
                                    {loading ||
                                    (label === "Gasto no Mês" && loadingMes) ||
                                    (label === "Compras no Mês" &&
                                        loadingMes) ? (
                                ...
                                <ArrowRight
                                    size={10}
                                    className="absolute bottom-4 right-4 text-(--muted-foreground) opacity-0 group-hover:opacity-40 transition-opacity"
                                />
                            </button>
                        ),
                    )}
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            <span
                                                className={`text-[0.5rem] font-black tabular-nums shrink-0 w-5 ${i < 3 ? "text-(--primary)" : "text-(--muted-foreground) opacity-40"}`}
                                            >
                    ...
                    <div className="bg-card border border-(--border) flex-1">
                        ...
                        <div className="p-4 flex flex-col gap-3">
                            {loading ? (
                                ...
                                <p className="text-[0.72rem] text-(--muted-foreground) text-center py-3">
                                    Sem compras no mês.
                                </p>
                            ) : (
                                filiais.map((f, i) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="flex items-center gap-2">
                                                <span
                                                    className={`text-[0.48rem] font-black tabular-nums shrink-0 w-4 ${i < 3 ? "text-(--primary)" : "text-(--muted-foreground) opacity-40"}`}
                                                >
                                                    {String(i + 1).padStart(
                                                        2,
                                                        "0",
                            ...
                            {[
                                ...
                                >
                                    <div className="flex items-center gap-2.5">
                                        ...
                                        />
                                        <span className="text-[0.65rem] font-semibold text-(--foreground) group-hover:text-(--primary) transition-colors">
                                            {label}
                                        </span>
                                    </div>
                                    <ArrowRight
                                        size={10}
                                        className="text-(--muted-foreground) opacity-0 group-hover:opacity-60 transition-opacity"
                                    />
                                </button>
                            ))}
                        </div>
